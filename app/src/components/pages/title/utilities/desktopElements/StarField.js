...
export default function StarField({ colour, active }) {
  ...
  useEffect(() => {
    ...
    const spawnComet = (width, height) => ({
      ...
      life: 0,
      active: false,
      // The same steep power as the scene uses: most comets sit still for a
      // long time, so two of them rarely fly at once.
    ...
    const resize = () => {
      ...
      if (comets.length === 0) {
        comets = Array.from({ length: COMET_COUNT }, () => spawnComet(width, height));
      }
    };
    resize();
